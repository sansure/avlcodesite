...
function updatePageTable(data){const tbody=document.getElementById('pageStatsBody');if(!tbody)return;if(data.length===0){tbody.innerHTML='<tr><td colspan="4" class="text-center">暂无数据</td></tr>';return}tbody.innerHTML=data.map(item=>\`<tr><td>\${item.page}</td><td>\${item.views}</td><td>\${item.visitors}</td><td>\${item.avg_time || '-'}</td></tr>\`).join('')}async function loadUsers(){try{const resp=await fetch('/admin/api/users');const users=await resp.json();const tbody=document.getElementById('userTableBody');if(!tbody)return;if(users.length===0){tbody.innerHTML='<tr><td colspan="4" style="text-align:center;color:var(--avl-text-secondary);padding:24px">暂无用户</td></tr>';return}tbody.innerHTML=users.map(u=>\`<tr><td>\${u.id}</td><td>\${u.username}</td><td>\${formatDate(u.created_at)}</td><td><button onclick="openEditModal(\${u.id})" class="btn btn-sm btn-primary" style="margin-right:8px">修改密码</button><button onclick="deleteUser(\${u.id})" class="btn btn-sm btn-danger">删除</button></td></tr>\`).join('')}catch(e){console.error('加载用户列表失败:',e)}}
function openCreateModal(){document.getElementById('newUsername').value='';document.getElementById('newPassword').value='';document.getElementById('newPasswordConfirm').value='';document.getElementById('createModal').classList.add('active')}
function closeCreateModal(){document.getElementById('createModal').classList.remove('active')}
async function createUser(){const username=document.getElementById('newUsername').value.trim();const password=document.getElementById('newPassword').value;const confirm=document.getElementById('newPasswordConfirm').value;if(!username||!password||!confirm)return alert('请填写完整');if(password!==confirm)return alert('两次输入的密码不一致');if(password.length<6)return alert('密码至少6位');const resp=await fetch('/admin/api/users',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username,password})});const data=await resp.json();if(resp.ok){closeCreateModal();loadUsers()}else{alert(data.error||'创建失败')}}
function openEditModal(userId){document.getElementById('editUserId').value=userId;document.getElementById('editPassword').value='';document.getElementById('editPasswordConfirm').value='';document.getElementById('editModal').classList.add('active')}
function closeEditModal(){document.getElementById('editModal').classList.remove('active')}
async function updateUser(){const userId=document.getElementById('editUserId').value;const password=document.getElementById('editPassword').value;const confirm=document.getElementById('editPasswordConfirm').value;if(!password||!confirm)return alert('请填写完整');if(password!==confirm)return alert('两次输入的密码不一致');if(password.length<6)return alert('密码至少6位');const resp=await fetch('/admin/api/users?id='+userId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({password})});const data=await resp.json();if(resp.ok){closeEditModal();loadUsers()}else{alert(data.error||'修改失败')}}
...
<body>
  ...
  <div id="createModal" class="modal-overlay">
    <div class="modal">
      ...
      <div class="form-group">
        <label>确认密码</label>
        <input type="password" id="newPasswordConfirm" placeholder="请再次输入密码">
      </div>
